...
function stubContextFromBindingDefinitions(bindingDefinitions, manipulator) {
    ...
    bindingDefinitions.forEach((definition) => {
        ...
        if (definition.direction.toLowerCase() === 'out') {
            handler.addOutputBinding(context, definition, definition);
        } else if (definition.type.substr(-7).toLowerCase() === 'trigger') {
            handler.addTrigger(context, definition, definition);
        } else {
            handler.addInputBinding(context, definition, definition);
        }
    });
